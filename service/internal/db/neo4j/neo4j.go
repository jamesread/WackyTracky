...
func (api Neo4jDB) GetSubtasks(itemId string) ([]db.DBTask, error) {
	var ret = make([]db.DBTask, 0)

	cql := "MATCH (p:Item)-->(i:Item) WHERE id(p) = $parentItemId RETURN p, i"

	itemIdNum, _ := strconv.Atoi(itemId)

	params := map[string]any{
		"parentItemId": itemIdNum,
	}

	for _, row := range readTxParams(cql, params) {
		parent := row.Values[0].(dbtype.Node)
		item := row.Values[1].(dbtype.Node)

		ret = append(ret, db.DBTask{
			ID:            fmt.Sprintf("%v", item.ElementId),
			ParentType:    "item",
			CountSubitems: 0,
			ParentId:      fmt.Sprintf("%v", parent.ElementId),
			Content:       fmt.Sprintf("%v", item.Props["content"]),
		})
	}
